...
/**
 * Borra una CUENTA sin ficha ni rol (RMR-TSK-0520): su login (/users) y su
 * usuario de Firebase Auth, por Cloud Function (borrar en Auth exige el Admin
 * SDK). La CF rechaza cuentas con ficha o con rol y explica qué quitar antes.
 * @param {string} uid
 * @returns {Promise<void>}
 */
export async function deleteAccount(uid) {
  const { httpsCallable } = await import('firebase/functions');
  const fn = httpsCallable(await getRegionalFunctions(), 'deleteAccount');
  await fn({ uid });
}
