...
describe('mergeVerdict — la decisión de mergear sale del estado, no de leer un texto', () => {
  ...
  it('aborta en cuanto uno falla, aunque otros sigan corriendo', () => {
    // Esperar al resto no cambiaría el desenlace: la PR ya no se puede mergear.
    const v = mergeVerdict({
      state: 'OPEN',
      statusCheckRollup: [action('E2E', 'COMPLETED', 'FAILURE'), action('Quality', 'IN_PROGRESS', null)],
    });
    expect(v.action).toBe('abort');
    expect(v.reason).toContain('E2E (FAILURE)');
  });
  ...
  it('no mergea un borrador aunque esté todo verde', () => {
    const v = mergeVerdict({
      state: 'OPEN',
      isDraft: true,
    ...
    expect(v.action).toBe('abort');
    expect(v.reason).toContain('borrador');
  });

  it('los conflictos con la base ganan a los checks que sigan corriendo', () => {
    const v = mergeVerdict({
      state: 'OPEN',
      mergeable: 'CONFLICTING',
      statusCheckRollup: [action('E2E', 'IN_PROGRESS', null)],
    });
    expect(v.action).toBe('abort');
    expect(v.reason).toContain('conflictos');
  });

  it('espera mientras GitHub no sepa aún si la rama se puede mergear', () => {
    // `UNKNOWN` es un estado intermedio: darlo por bueno sería mergear a ciegas.
    const v = mergeVerdict({
      state: 'OPEN',
      mergeable: 'UNKNOWN',
      statusCheckRollup: [action('Quality', 'COMPLETED', 'SUCCESS')],
